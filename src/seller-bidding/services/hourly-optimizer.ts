...
import { buildDeliveryWindow, buildValidityWindow, buildHourlyPublishRequest, extractHourlyIds, buildHourlyCatalog } from './hourly-catalog-builder';
import { fetchMarketData, calculatePrice } from '../../bidding/services/market-analyzer';
import { analyzeCompetitorsForHour } from './hourly-market-analyzer';

const PUBLISH_URL = process.env.PUBLISH_URL || 'http://localhost:3002/api/publish';
 ...
 */
export async function preview(request: SellerBidRequest): Promise<SellerPreviewResponse> {
  ...
  const { valid: validHours, skipped: skippedHours } = filterValidHours(forecast);
  ...
  try {
    competitorOffers = await fetchMarketData(targetDate, targetDate, request.source_type);
    console.log(`[SellerBidding] Found ${competitorOffers.length} competitor offers`);
  } catch (error: any) {
    console.log(`[SellerBidding] CDS query failed: ${error.message}, using floor price`);
  }
  ...
  for (const hourData of validHours) {
    ...
    const expectedRevenue = calculateExpectedRevenue(hourData.excess_kwh, price);
  ...
  const sortedBids = [...allBids].sort((a, b) => b.expected_revenue_inr - a.expected_revenue_inr);
  const selectedBids = sortedBids.slice(0, TOP_N_HOURS);

  // Log selection decision
  console.log(`[SellerBidding] Selected top ${selectedBids.length} hours by revenue:`);
  for (const bid of selectedBids) {
    console.log(`  ${bid.hour}: ${bid.quantity_kwh} kWh @ ${bid.price_inr} INR = ${bid.expected_revenue_inr} INR`);
  }
  ...
  // Calculate summary
  const totalQuantity = selectedBids.reduce((sum, b) => sum + b.quantity_kwh, 0);
  const totalRevenue = selectedBids.reduce((sum, b) => sum + b.expected_revenue_inr, 0);
 ...
 */
export async function confirm(request: SellerBidRequest): Promise<SellerConfirmResponse> {
  const targetDate = getTomorrowDate();
  ...
  for (const bid of previewResult.bids) {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        console.log(`[SellerBidding] Published bid for ${bid.hour}: catalog=${ids.catalog_id}`);
        placedBids.push({
          hour: bid.hour,
      ...
      console.error(`[SellerBidding] Failed to publish bid for ${bid.hour}: ${errorMsg}`);
  ...
  console.log(`[SellerBidding] Confirm complete: ${placedBids.filter(b => b.status === 'PUBLISHED').length}/${previewResult.bids.length} published`);
